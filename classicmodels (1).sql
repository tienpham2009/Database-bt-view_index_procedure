...
--
-- Host: localhost
-- Generation Time: May 31, 2021 at 08:49 PM
...
-- Procedures
--
CREATE DEFINER=`root`@`localhost` PROCEDURE `findAllCustomers` (IN `number` INT)  BEGIN

   SELECT *  FROM customers where customerNumber = number;

END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `getCustomerByCity` (IN `city` VARCHAR(50), OUT `total` INT)  BEGIN 
 
SELECT COUNT(customers.customerNumber) INTO total FROM customers
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `GetOrderCountByStatus` (IN `orderStatus` VARCHAR(255), OUT `total` INT)  BEGIN
...
(112, 'Signal Gift Stores', 'King', 'Jean', '7025551838', '8489 Strong St.', NULL, 'getCustomerBy', 'NV', '83030', 'USA', 1166, '71800.00'),
...
ALTER TABLE `customers`
  ...
  ADD KEY `salesRepEmployeeNumber` (`salesRepEmployeeNumber`) USING BTREE,
